...
class ClickAttribute:
    ...
    def serialize_schema(self):
        return avro.loads(str(self.schema))
...
class ClickEvent:
    ...
    def serialize_schema(self):
        return avro.loads(json.dumps(self.schema))
...
async def consume(topic_name):
    ...
    c = AvroConsumer(
        config={"bootstrap.servers": BROKER_URL, "group.id": "0"},
        schema_registry=schema_registry
    )
...
def main():
    ...
    try:
        asyncio.run(produce_consume("kafka.schema.registry"))
